...
1. Download or inspect `https://tubealfred.com/openapi.json` from a trusted connection.
...
4. Run `python3 scripts/sync_contract.py --openapi <snapshot> --pinned-on YYYY-MM-DD`.
...
The weekly CI job checks the pin against the official OpenAPI document. A drift failure is a review signal, not permission to regenerate blindly.
...
- The catalog date matches the reviewed OpenAPI source.
